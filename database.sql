...
SET "class" =
	...
	CASE
		...
		WHEN name IN ('K''Sante')
			THEN ARRAY['Warden', 'Skirmisher']
		
	------- Cases where champion has 1 class -------	
		WHEN name IN ('Janna', 'Lulu', 'Milio', 'Nami', 'Renata Glasc', 'Sona', 'Soraka', 'Yuumi')
		...
		WHEN name IN ('Bel''Veth', 'Fiora', 'Gwen', 'Jax', 'Kayn', 'Kled', 'Lillia', 'Master Yi', 'Nilah', 'Riven', 'Tryndamere', 'Viego', 'Yasuo')
...
SET "region" =
	CASE
		...
		WHEN name IN ('Fizz', 'Gangplank', 'Graves', 'Illaoi', 'Miss Fortune', 'Nautilus', 'Nilah', 'Pyke', 'Tahm Kench', 'Twisted Fate')
			THEN 'Bilgewater'
		...
		WHEN name IN ('Malphite', 'Milio', 'Neeko', 'Nidalee', 'Qiyana', 'Rengar', 'Zyra')
			THEN 'Ixtal'
		...
		WHEN name IN ('Akshan', 'Amumu', 'Azir', 'K''Sante', 'Nasus', 'Rammus', 'Renekton', 'Rengar', 'Sivir', 'Skarner', 'Taliyah', 'Xerath')
			THEN 'Shurima'
		...
		WHEN name IN ('Bel''Veth', 'Cho''Gath', 'Kai''Sa', 'Kassadin', 'Kha''Zix', 'Kog''Maw', 'Malzahar', 'Rek''Sai', 'Vel''Koz')
			THEN 'The Void'
